...
python -m scripts.time_travel --start 2025-06-01 --end 2025-12-01 --symbols BTCUSDT ETHUSDT SOLUSDT

# Custom threshold sweep
python -m scripts.time_travel --start 2025-01-01 --thresholds 2.5,3.5,4.5,6.5

# Dry run — preview without writing to DB
python -m scripts.time_travel --start 2025-01-01 --symbols BTCUSDT --dry-run

# Verbose logging
python -m scripts.time_travel --start 2025-01-01 --log-level DEBUG
...
The script extracts the scoring logic from `enzymes/score_confluence.py` into standalone functions (in `scripts/time_travel/scoring.py`). Same weights, same formula constants, same momentum cap/dampening, same cross-timeframe alignment check. A bar that scores 6.3 in the live daemon scores 6.3 in time travel.
...
| `--min-threshold` | 60% of `entry_threshold` | Skip trades below this threshold |
| `--equity` | from config | Starting equity for position sizing |
| `--fee-rate` | from config | Exchange fee rate per side |
| `--per-symbol` | `false` | Include per-symbol breakdown in output |
...
```
scripts/time_travel/
  ├── __main__.py                  → CLI entry point (python -m scripts.time_travel)
  ├── scoring.py                   → compute_confluence_score(), build_signals_at_entry()
  ├── simulation.py                → simulate_exit(), compute_backtest_trade(), TradeCooldown
  ├── data.py                      → fetch_historical_ohlcv(), precompute_indicators()
  ├── reporting.py                 → compute_aggregate_metrics(), format_summary_table/json()
  └── export_trades.py             → CSV export (see docs/README_EXPORT_TRADES.md)
      │
      └── Depends on project modules:
          ├── indicators/registry.py        → compute_indicator()
          ├── enzymes/score_confluence.py   → scoring formula (mirrored in scoring.py)
...
The module does **not** build a full `Substrate` per bar (too heavy). Instead, it extracts the scoring logic into standalone functions that mirror `ScoreConfluence` exactly.
...
TIME TRAVEL — Fast-forward daemon
  ...
  Equity: $1000, Fee rate: 0.0006 (taker)
  Dry run: False
======================================================================
--------------------------------------------------
Processing BTCUSDT
...
TIME TRAVEL COMPLETE
  ...
  Total Return: +42.3%, Profit Factor: 1.87
  Expectancy/Trade: $8.12, Win/Loss Ratio: 2.14
  Max Drawdown: -12.4%, Total Fees: $31.50
...
| `pnl_usdt` | Dollar-math (position size × P&L, net of fees) |
...
```json
{
  ...
  "_source": "time_travel",
  "_position_size_usd": 250.0,
  "_gross_pnl_usd": 12.50,
  "_net_pnl_usd": 11.20,
  "_total_fees_usd": 0.30,
  "_atr_cap_applied": false
}
```
