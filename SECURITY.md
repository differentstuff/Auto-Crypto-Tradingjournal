...
This is a personal self-hosted tool, not a commercial product.
Security reports are welcome and will be reviewed when time allows —
but there are no response-time guarantees.
...
This app is designed for **personal use on a local network only**.
It has no authentication layer. Do not expose it to the public internet.
