...
async function journalLoad(page) {
  ...
  document.getElementById('journal-tbody').innerHTML = positions.map(t => `
    <tr onclick="openNotesModal(${t.id}, '${escHtml(t.notes||'')}', '${escHtml(t.tags||'')}', '${escHtml(t.analyst||'')}')"
        style="cursor:pointer" title="Click to edit analyst, notes & tags">
      <td><strong>${t.symbol}</strong></td>
      <td><span class="badge ${t.direction.toLowerCase()}">${t.direction}</span></td>
...
// ── Edit Notes Modal ───────────────────────────────────────────────────────────
function openNotesModal(id, notes, tags, analyst) {
