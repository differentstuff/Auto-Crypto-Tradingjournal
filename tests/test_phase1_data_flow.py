...
class TestConfigKeys:
    """P2/P8: New config keys are present in strategy YAMLs."""
    ...
    def test_default_yaml_has_lookback_hours(self):
        ...
        assert "trajectory_lookback_hours" in config.get("learning", {})
        assert "trajectory_min_hours" in config.get("learning", {})
        assert "trajectory_lookback_bars" not in config.get("learning", {})

    def test_momentum_rising_yaml_has_lookback_hours(self):
        """momentum_rising.yaml uses trajectory_lookback_hours."""
        import yaml
        with open(os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "config", "strategies", "momentum_rising.yaml"
        )) as f:
            config = yaml.safe_load(f)
        assert "trajectory_lookback_hours" in config.get("learning", {})
        assert "trajectory_min_hours" in config.get("learning", {})
    ...
    def test_paper_learning_test_yaml_has_lookback_hours(self):
        """paper_learning_test.yaml uses trajectory_lookback_hours."""
        import yaml
        with open(os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "config", "strategies", "paper_learning_test.yaml"
        )) as f:
            config = yaml.safe_load(f)
        assert "trajectory_lookback_hours" in config.get("learning", {})
        assert "trajectory_min_hours" in config.get("learning", {})

    def test_paper_test_yaml_has_learning_section(self):
        """paper_test.yaml now has a learning section with trajectory config."""
        import yaml
        with open(os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "config", "strategies", "paper_test.yaml"
        )) as f:
            config = yaml.safe_load(f)
        assert "trajectory_lookback_hours" in config.get("learning", {})
        assert "trajectory_min_hours" in config.get("learning", {})

    def test_no_retrain_every_hours_in_any_config(self):
        """P3: retrain_every_hours is removed from all configs."""
        import yaml
        config_files = [
            "config/default.yaml",
            "config/strategies/momentum_rising.yaml",
        ...
        for cf in config_files:
            with open(os.path.join(base_dir, cf)) as f:
                config = yaml.safe_load(f)
            assert "retrain_every_hours" not in config.get("learning", {}), \
                f"retrain_every_hours found in {cf}"